...
accuracy = resnet_scores[1]
loss = resnet_scores[0]
base_xception = Xception(weights='imagenet', include_top=False, input_shape=(48, 48, 3))
x = GlobalAveragePooling2D()(base_xception.output)
x = Dense(512, activation='relu')(x)
x = Dropout(0.5)(x)
output_x = Dense(7, activation='softmax')(x)
model_xception = Model(inputs=base_xception.input, outputs=output_x)
model_xception.compile(optimizer=Adam(1e-4), loss='categorical_crossentropy', metrics=['accuracy'])
model_xception.fit(X_train, y_train, epochs=100, validation_data=(X_test, y_test), callbacks=callbacks)
resnet_preds = model_resnet.predict(X_test)
xception_preds = model_xception.predict(X_test)
ensemble_preds = (resnet_preds + xception_preds) / 2
ensemble_acc = np.mean(np.argmax(ensemble_preds, axis=1) == np.argmax(y_test, axis=1))


print("Accuracy: {:.2f}% | Loss: {:.4f}".format(ensemble_acc * 100, loss))
