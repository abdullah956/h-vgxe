...
model_vgg16 = Model(inputs=base_vgg16.input, outputs=output_x)
model_vgg16.compile(optimizer=Adam(1e-4), loss='categorical_crossentropy', metrics=['accuracy'])
model_vgg16.fit(X_train, y_train, epochs=100, validation_data=(X_test, y_test), callbacks=callbacks)
resnet_preds = model_resnet.predict(X_test)
vgg16_preds = model_vgg16.predict(X_test)
ensemble_preds = (resnet_preds + vgg16_preds) / 2
ensemble_acc = np.mean(np.argmax(ensemble_preds, axis=1) == np.argmax(y_test, axis=1))


print("Accuracy: {:.2f}% | Loss: {:.4f}".format(ensemble_acc * 100))